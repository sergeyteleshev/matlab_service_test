...
function [factor_p, factor_Tq, factor_Ts, factor_Nq, factor_Ns, factor_Ca, factor_Cr, T, U_condition, Q_amount] = modelSystem(A, S, Pr, device_number, queue_max)        
    ...
    %%%%%%%выполнение текущей (S_MIN)
        ...
        if S_MIN < A(i)  %Следующее событие - выполнение имеющейся заявки
            ...
            if ~all(U) %Если есть хоть один 0 в массиве U - устройтво свободно
                ...
                if Pr(i)==4
                    k = find(U_priority==1);%Поиск устройства с низким приоритетом
                    if isempty(k)
                        k = find(U_priority==2);%Поиск устройства со средним приоритетом=2
                    end;
                    if isempty(k)
                        k = find(U_priority==3);%Поиск устройства со средним приоритетом=3
                    end;
                    if ~isempty(k)%Устройство с низким и средним приоритетом есть
                            ...
                            if length(Q)<queue_max %Проверка очереди
                                    ...
                                    %Добавление высокой заявки в начало очереди
                                    Q=[S(i),Q];
                                    Q_current=[i,Q_current];
                                    Q_priority=[Pr(i),Q_priority];

                                    %disp(sprintf('Заявка c высоким приоритетом добавлена в очередь. Осталось мест в очереди: %g', queue_max-length(Q)))
                                    ...
                                    %disp('Очередь переполнена. Заявка отклонена');
                        ...
                        end%Устройство с низким и средним приоритетом есть                    
                else
                    if Pr(i)==3
                        k = find(U_priority==1);%Поиск устройства с низким приоритетом
                        ...
                        if isempty(k)
                            k = find(U_priority==2);%Поиск устройства со средним приоритетом приоритетом
                        end;
                        ...
                        if Pr(i)==2
                            ...
                            if ~isempty(k)%Устройство с низким приоритетом есть
                                ...
                                if length(Q)<queue_max %Проверка очереди
                                    ...
                                    Q_current=[i,Q_current];
                                    Q_priority=[U_priority(k(1)),Q_priority];
                                    %disp(sprintf('Заявка c низким приоритетом перемещена в очередь. Осталось мест в очереди: %g', queue_max-length(Q)))
                            ...
                            if length(Q)<queue_max %Проверка очереди
                                ...
                                Q(end+1)=S(i);
                                Q_priority(end+1)=Pr(i);
                                Q_current(end+1)=i;

                                %disp(sprintf('Заявка с низким приоритетом добавлена в очередь. Осталось мест в очереди: %g', queue_max-length(Q)))
    ...
    factor_Ca = (length(A))/T(end);                             %Абсолютная пропускная способность системы
    factor_Cr = (Ns)/length(A);                          %Относительная пропускная способность системы
